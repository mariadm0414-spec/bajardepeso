...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const completeOnboarding = (data: OnboardingData) => {
        ...
        setAppState(prev => {
            if (prev.startDate) return prev; // already started — don't reset
            return {
                ...prev,
                startDate: now,
    ...
    return (
        <AuthContext.Provider value={{ user, login, logout, completeOnboarding, upgradePremium }}>
            <AppContext.Provider value={{ appState, challengeDay, startChallenge, completeMeal, uncompleteDay, logWeight, getDayCompletion, todayProgress }}>
                {children}
            </AppContext.Provider>
